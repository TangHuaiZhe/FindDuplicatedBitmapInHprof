...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        String HPROF_PATH = "/Users/tang/Downloads/memory-20191012T162805.hprof";
        File hprofFile = new File(HPROF_PATH);
        if(!hprofFile.exists()){
            System.out.print("file: " + HPROF_PATH + " not exist, please check it");
            return;
        }
        Map<String, ArrayList<ObjNode>> objMap = new HashMap<>();
    ...
    private static void dumpSameBufferBitmapInfo(Map<String, ArrayList<ObjNode>> map){
        ...
        for (Entry<String, ArrayList<ObjNode>> stringArrayListEntry : map.entrySet()) {
            if (((ArrayList) ((Entry) stringArrayListEntry).getValue()).size() > 1) {
                try {
                    ArrayList<ObjNode> objNodeArrayList = (ArrayList) ((Entry) stringArrayListEntry).getValue();
                    JSONObject jsonObject = new JSONObject();
                    jsonObject.put("duplcateCount", objNodeArrayList.size());
                    jsonObject.put("bufferHash", ((Entry) stringArrayListEntry).getKey());
                    jsonObject.put("width",
                        (int) HahaHelper.fieldValue(
                            ((ClassInstance) objNodeArrayList.get(0).getInstance()).getValues(),
                            "mWidth"));
        ...
        System.out.print("result: " + jsonResult.toString());
    }
}
